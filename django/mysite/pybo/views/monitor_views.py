...
def monitor_list(request, project_id):
    ...
    if request.method == "POST":
        ...
        request_url = "https://10.0.0.79:6443/api/v1/namespaces/logging/pods/"# .format(request.POST['PN'])
        ...
        request_url = "https://10.0.0.79:6443/api/v1/namespaces/logging/services/"# .format(request.POST['PN'])
        headers = {"Authorization": "Bearer {}".format(token)}
        api_response = requests.get(request_url, headers=headers, verify=False)
        api_json = api_response.json()
